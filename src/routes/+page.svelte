...
<div class="flex h-full flex-col">
	...
	<main class="min-h-0 flex-1">
		...
		{:else if ui.view === 'library'}
			<div class="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
				<h2 class="text-xl font-medium">Your library is empty</h2>
				<p class="max-w-md text-sm text-zinc-500">
					{library.books.length} books. EPUB import arrives with the reader core (Phase B).
				</p>
			</div>
		{:else}
			<!-- ReaderView mounts here in Phase B -->
			<div></div>
		{/if}
	</main>
</div>
