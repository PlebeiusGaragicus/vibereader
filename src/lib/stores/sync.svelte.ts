...
async function restoreBook(sha256: string): Promise<void> {
	...
	try {
		...
		const servers = book?.blossom?.servers ?? settingsStore.settings.blossomServers;
		const blob = await downloadBlob(servers, sha256, '.epub');
		await db.bookFiles.save({ sha256, blob });
		if (book?.blossom?.coverSha256) {
			await downloadBlob(servers, book.blossom.coverSha256)
				.then((c) => db.covers.save({ sha256, blob: c, mimeType: c.type || 'image/jpeg' }))
				.catch(() => {});
		}
		await library.refresh();
